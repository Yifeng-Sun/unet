...
def getModel(args):
    if args.arch == 'Attention_UNet' or 'att':
        model = AttU_Net(3, 1).to(device)
    if args.arch == 'UNet_3Plus' or 'u3':
        model = UNet_3Plus(3, 1, 4, True, True).to(device)
        ...
        for pic, _, pic_path, mask_path in val_dataloaders:
            ...
            if save_predict == True:
                if args.dataset == 'driveEye':
                    ...
                    saved_predict_path = dir + '/' + mask_path[0].split('\\')[-1]
                    saved_predict = '.' + saved_predict_path.split('.')[1] + '.tif'
                    os.makedirs(saved_predict_path)
                    f = open(saved_predict, 'w')
                    f.close()
                    plt.savefig(saved_predict)
